...
D.1 - Lors de l'affichage du premier écran, sélectionner la première ligne  
...
D.2 - Taper `yes` lorsque cela vous est demandé
<br><br>
...
D.3 - Renseigner le nom d'utilisateur pour vous connecter à la console  
...
D.4 - Renseigner un mot de passe  
...
D.5 - Re-valider le mot de passe
<br><br>
...
D.6 - Après plusieurs ligne de défilement, cette écran vous invite à redémarrer en pressant `Entrée`
<br><br>
...
D.7 - A cet écran, taper le nom d'utilisateur rensigner à l'étape 4.3
<br><br>
...
D.8 - Puis votre mot de passe de l'étape 4.4
<br><br>
...
D.9 - Sélectionner `Yes` pour continuer
<br><br>
...
D.10 - Sélectionner `Install` puis `ok`
<br><br>
...
D.11 - Sélectionner la version `STANDALONE` puis `ok`
<br><br>
...
D.12 - Renseigner `AGREE` dans le champ puis `ok`
<br><br>
...
D.13 - Choisir l'option adéquate puis `ok`  
...
D.14 - Renseigner un nom d'hôte pour le serveur  
...
D.15 - Si vous avez gardé le choix précédent par défaut, sélectionner `Use Anyway`
<br><br>
...
D.16 - Renseigner une courte desciption de votre noeud puis `ok`  
...
D.17 - Sélectionner la carte réseau de gestion (celle pour vous connecter au webui) puis `ok`
<br><br>
...
D.18 - Sélectionner `STATIC` afin d'attribuer une adresse IP fixe à la carte de gestion puis `ok`  
...
D.19 - Donner une adresse IP à l'interface de gestion en spécifiant le CIDR puis `ok`
<br><br>
...
D.20 - Spécifier la passerelle puis `ok`
<br><br>
...
D.21 - Spécifier le(s) server(s) DNS (il est possible de garder ceux par défaut) puis `ok`
<br><br>
...
D.22 - Spécifier un domaine de recherche DNS (il est possible de garder celui par défaut) puis `ok`
<br><br>
...
D.23 - Sélectionner l'option adéquate puis `ok`  
...
D.24 - Sélectionner l'option adéquate puis `ok`  
...
D.25 - Ajouter le(s) différente(s) carte réseaux qui serviront au monitoring avec `barre espace` puis `ok`
<br><br>
...
D.26 - Renseigner une adresse mail qui servira à l'authentification sur la webui puis `ok`
<br><br>
...
D.27 - Taper un mot de passe puis `ok`
<br><br>
...
D.28 - Re-taper votre mot de passe puis `ok`
<br><br>
...
D.29 - Choisir l'option adéquate puis `ok`  
...
D.30 - Autorisez vous l'accès à la configuration via la webui, choisir `ok`
<br><br>
...
D.31 - Définir une adresse IP autorisée, ou une plage d'adresse en spécifiant le CIDR pour l'accès à la webui.
<br><br>
...
D.32 - Soutenez la communauté en validant `Yes`  
...
D.33 - Vérifier les informations, si ok appuyer sur `TAB` pour sélectionner `Yes` puis `Entrée`  
...
D.34 - Cette écran s'affiche à la fin de l'installation, il récapitule les informations de connexion, appuyer `TAB` puis `ok`
<br><br>
...
D.35 - Après le redémarrage du serveur, connecter vous avec le nom d'utilisateur de l'étape 4.3
<br><br>
...
D.36 - Puis votre mot de passe de l'étape 4.4
<br><br>
...
D.37 - Si votre connection est valide vous avez l'écran suivant, indiquant l'adresse de connexion de la webui
<br><br>
